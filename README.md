...
>
> 当赛场上电机掉线而7分钟比赛时间无法上电检修时，你就会意识到日志的重要性

---

## 📁 项目文件

- `串口记录仪1.3.epro2` - 硬件工程文件
- `TF_Log.elf` - TF卡日志模块固件
- `RP_Log_master/RP_Log.c .h` - 主机端日志代码

---

## 📚 文档导航

- **[主机端日志系统移植指南](RP_Log_master/RP_Log_README.md)**
...
**日志文件示例：**
![alt text](image/file_example.png)
...
- ✅ **快速查找最新日志**: 日志模块断电重启自动创建一个新日期的日志文件，若日期时间重复则自动编号文件名，方便快速寻找最新日志文件
- ✅ **自动填充时间戳**: `[月_日][时_分_秒]......`
- ✅ **自动保存**: 定时同步文件到tf卡，避免丢日志
- ✅ **多路输出**: 同时输出到RTT和文件，并且使用串口助手连接typeC口即可实时查看输出日志。由数据流向可知，此模块可同时作为临时usb转ttl使用。
...
5. 装上纽扣电池，插上usb，并使用串口助手发送`CMD:SET_TIME=20141228,010200`命令配置成当前时间（当前版本需在正确装载TF卡的情况下才能设置时间，待后续修复）
6. 移植RP_LOG日志库，体验自动填充`[日志等级][文件:行号]:`等功能
...
1. 按照快速开始操作后，TF卡没有新建文件怎么办？
   - 请检查此TF卡是否支持SPI读写，若不支持请更换
   - 请检查此TF是否是FAT32文件系统，若不是请修改
...
5. 打开日志文件发现没有日志怎么办？
   ...
   - 检查日志末尾是否有`\r\n`帧尾，日志模块通过检测帧尾来保存单条日志。

---
## 注意事项
1. 由于热插拔TF卡可能造成损坏，故需要定期检查TF卡是否正常，若出现大量.xx隐藏文件夹或无法正常创建文件，请尝试格式化TF卡。
2. 建议比赛前检查TF卡是否正常。
...
- 快速检索工具：按日志等级（ERROR、INFO等）快速筛选
- 可视化波形：按时间排列显示不同等级日志数量的波形，不同等级显示不同颜色
- 轻量化电脑：一台开机即用的电脑，笔记本上git同步所有人代码，支持快速修改、烧录、查看日志
